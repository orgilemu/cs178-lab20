...
def build_my_chart(df, display_name, types):
    ...
    # Get the type color (fallback to gray if type isn't in the dict)
    hex_color = TYPE_COLORS.get(types[0], "#A8A878")
    
    # Sort values from lowest to highest so largest bar is at the top
    ...
    fig = px.bar(
        ...
        y="stat", 
        orientation="h",
        title=f"{display_name} — Sorted Base Stats",
        labels={"value": "Base Stat Value", "stat": ""},
